...
```{r}
bxd_results <- haplotype_reconstruction_pipeline('BXD', list_pheno, qtl2_file_gen = T, samples_gen = T)
...
```{r}
f2_results <- haplotype_reconstruction_pipeline('F2', list_pheno, qtl2_file_gen = T, samples_gen = F)
```
...
R/qtl2: 
  ...
  2. disadvantages:
      ...
      b. does not scale well with large number of founders, run time increases exponentially 
      c. visualizations are less detailed and need to be generated on demand
